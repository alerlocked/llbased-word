#!/bin/bash
# ============================================================================
# Qwen3-30B-A3B (Simple Tier) - MindIE 2.2 RC1 on Atlas 300I Duo
#
# Chips: 4-7 (physical) | Port: 1028 | worldSize: 4
# Container: mindie-qwen3-30b-a3b
#
...
#
# Prerequisites:
#   - Model at /home/data/Qwen3-30B-A3B
#   - MindIE image loaded
#   - NPU driver/firmware installed on host
#
# Usage:
#   chmod +x run-qwen3-30b-a3b.sh
#   ./run-qwen3-30b-a3b.sh          # start
#   ./run-qwen3-30b-a3b.sh stop     # stop
#   ./run-qwen3-30b-a3b.sh test     # test endpoint
#   ./run-qwen3-30b-a3b.sh logs     # view daemon logs
# ============================================================================
...
CONTAINER_NAME="mindie-qwen3-30b-a3b"

case "${1:-start}" in
    start)
        echo "[INFO] Starting Qwen3-30B-A3B (chips 4-7, port 1028)..."
        ...
        python3 -c "
import json, sys
path = '$MODEL_DIR/Qwen3-30B-A3B/config.json'
try:
...
"
        ...
        # Run container
        # Use --net=host + --privileged to let ACL enumerate all devices.
        # Isolation is done by ASCEND_RT_VISIBLE_DEVICES=4,5,6,7 in daemon,
        # which restricts this container to only use chips 4-7 at runtime.
        # Qwen3-14B container uses chips 0-3 via the same mechanism.
        docker run -d \
            --name $CONTAINER_NAME \
...
mdc["maxSeqLen"] = 32768
mdc["maxInputTokenLen"] = 28672
mdc["truncation"] = False

mc = {
    "modelInstanceType": "Standard",
    "modelName": "qwen3-30b-a3b",
    "modelWeightPath": "/models/Qwen3-30B-A3B",
    "worldSize": 4,
    "cpuMemSize": 5,
    "npuMemSize": -1,
    "backendType": "atb",
...
'
        ...
        echo "[INFO] Starting MindIE daemon (chips 4-7, port 1028)..."
...
ls -la /models/Qwen3-30B-A3B/config.json >> $LOG 2>&1
...
chmod +x /tmp/start_daemon.sh'
    ...
    test)
        echo "[INFO] Testing Qwen3-30B-A3B on port 1028..."
        echo ""
        echo "=== /v1/models ==="
        curl -s --max-time 5 http://localhost:1028/v1/models 2>/dev/null || echo "Not responding (may still be loading)"
        echo ""
        echo ""
        echo "=== Inference test ==="
