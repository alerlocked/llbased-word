...
def test_parse_methods(parser):
    ...
    for method in methods:
        if hasattr(parser, method):
            is_async = asyncio.iscoroutinefunction(getattr(parser, method))
            async_mark = " (async)" if is_async else ""
            print(f"[OK] 方法 {method}{async_mark} 存在")
        else:
...
async def test_parse_real_pdf_async(parser):
    """测试4: 实际PDF解析测试（异步版本）"""
    print("\n" + "="*60)
    print("测试4: 实际PDF文件解析")
    print("="*60)
...
def test_api_endpoint():
    ...
    for endpoint in endpoints:
        try:
            ...
            if response.status_code == 200:
                print(f"[OK] 端点正常: {endpoint}")
            elif response.status_code == 405:
                print(f"[OK] 端点存在: {endpoint} (需要POST方法)")
            elif response.status_code == 404:
                print(f"[WARN] 端点不存在: {endpoint}")
            elif response.status_code == 500:
                print(f"[WARN] 端点错误: {endpoint} (500内部错误)")
                try:
...
async def test_pdf_upload_api():
    """测试6: PDF上传API测试"""
    print("\n" + "="*60)
    print("测试6: PDF上传解析API测试")
    print("="*60)
    ...
    if not test_pdf:
        print("[WARN] 未找到测试PDF文件，跳过API上传测试")
        return None

    print(f"[INFO] 使用测试PDF: {test_pdf.name}")

    # 测试上传端点
    upload_endpoints = [
        "/api/documents/parse",
        "/api/process-documents/upload",
    ]
    ...
    for endpoint in upload_endpoints:
        try:
            ...
            if response.status_code == 200:
                print(f"[OK] {endpoint} 上传成功")
                result = response.json()
                print(f"      返回键: {list(result.keys())[:5]}")
                return True
            else:
                print(f"[WARN] {endpoint} 失败: {response.status_code}")
        except Exception as e:
            print(f"[INFO] {endpoint} 测试: {e}")

    return False

async def main():
    print("\n" + "="*60)
    print("PDF解析功能组件测试 v2")
    print("="*60)
    ...
    # 测试4: 实际PDF解析（异步）
    success = await test_parse_real_pdf_async(parser)
    ...
    # 测试6: PDF上传API
    success = await test_pdf_upload_api()
    results.append(("PDF上传API", success if success is not None else True))
