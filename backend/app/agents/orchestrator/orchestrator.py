    ...
    async def _execute_chapters_parallel(
        ...
        for title in unique_titles:
            ...
            if sub_sources:
                    ...
                    task = {
                        ...
                        "params": {
                            ...
                            "module_instruction": (
                                ...
                                "\n"
                                "重要原则——这是生成系统，不是摘抄系统：\n"
                                "参考文档仅用于了解格式、术语和参数风格，不要照搬原文。\n"
                                "\n"
                                "要求：\n"
                                "1. 参考知识库文档的格式和术语体系，根据工艺要求生成规范的工艺文件内容\n"
                                "2. 关键技术参数（代号、材料牌号、公差等）必须准确；参考文档中有的直接使用\n"
                                "3. 描述性文字用自己的语言组织，不要直接复制原文段落\n"
                                "4. 如果原文包含表格数据（工艺参数表、材料清单等），以表格形式输出\n"
                                "5. 工序按编号顺序输出，每道工序包含：工序名称、设备/工具、操作内容、检验要求\n"
                                "6. 必须排除文档噪声：签名栏（编制/校对/审核/批准及人名）、日期戳、页码、更改单号、续表标记\n"
                                "7. 不要输出分析过程、来源说明、页码引用或对原文内容的点评"
                            ),
                            "skip_planning": True,
                        },
                        "generate_doc": False,
                ...
                if has_source:
                    instruction = (
                        f"你是工艺文件编写专家。请参考知识库文档，为「{title}」章节生成规范的工艺文件内容。\n"
                        "\n"
                        "重要原则——这是生成系统，不是摘抄系统：\n"
                        "知识库文档仅供参考格式、术语和参数风格。你需要基于工艺要求生成内容，而不是照搬原文。\n"
                        "\n"
                        "要求：\n"
                        "1. 参考知识库文档的格式规范和术语体系，根据用户描述的工艺要求生成内容\n"
                        "2. 关键技术参数（代号、材料牌号、公差等）必须准确；如果参考文档中有，直接使用\n"
                        "3. 工序按编号顺序输出，描述性文字用自己的语言组织\n"
                        "4. 不要直接复制原文段落，尤其是不要把 PDF 表头、签名栏、日期戳、页码当作数据输出\n"
                        "5. 每道工序应包含：工序名称、设备/工具、操作内容、检验要求（如适用）\n"
                        "6. 如果包含表格数据（工艺参数表、材料清单等），以标准 Markdown 表格输出\n"
                        "7. 必须排除以下噪声内容（它们是文档元数据，不是工艺数据）：\n"
                        "   - 签名栏（编制/校对/审核/标检/批准，以及人名如'牛超'等）\n"
                        "   - 日期戳（如 20240828）\n"
                        "   - 页码、页数、更改单号\n"
                        "   - 续表标记（如'XX(续)'、'配套明细表(续)'）\n"
                        "   - 产品型号/图号出现在非数据字段中时（如作为签名栏占位）\n"
                        "8. 不要输出分析过程、来源说明或对原文的点评"
                    )
                else:
