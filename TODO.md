...
### 3f. 📌 画像体系语义拍板（2026-08-22 用户定，待 lead 立项实现）
- **画像定位**：跨素材的通用经验，每个项目可选取使用，非项目专属
- **维度**：按专业分库（已有）+ 内容按来源（素材/型号）标注隔离（**待实现**——triples/knowledge 条目带来源维度，与工作区勾选联动可选取用）
- **原则/细则**：通用工艺原则内容不变，跟着画像体系走不单独一套
- **学习入口**：预设控制，暂不给用户自选经验功能；当前**冻结不学新内容**（学习为手动入口，不点不学，现状已满足）
- **生成取用**：用哪个专业注哪个专业画像（已实现 orchestrator:353）
- 实现走 lead 流程（PLAN 封板+审批）；关联 3d learn-feedback 项目级专业字段尾巴
